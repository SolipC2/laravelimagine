 ...
 * - Video: XaiVideoClient (laravel/ai has no video gateway yet)
 */
class ImagineService
{
    ...
    public function generateImage(string $prompt, array $options = []): ImagineResult
    {
        ...
        try {
            ...
            if ($binary !== false && config('imagine.store_images', true)) {
                $ext = str_contains($mime, 'png') ? 'png' : 'jpg';
                $disk = (string) config('imagine.disk', 'public');
                $path = trim((string) config('imagine.image_path', 'imagine'), '/').'/'.uniqid('img_', true).'.'.$ext;
    ...
    /**
     * Unified entry: mode image|video.
     *
     * @param  array<string, mixed>  $options
     */
